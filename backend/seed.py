...
def seed_behaviors():
    ...
    behaviors_data = [
        {
            "name": "Juguetón",
            "desc": "Le encanta jugar y necesita actividad física regular.",
        },
        {
            "name": "Tranquilo",
            "desc": "Prefiere un ambiente calmado y disfruta de largas siestas.",
        },
        {
            "name": "Sociable",
            "desc": "Se lleva bien con otros perros y le gusta conocer gente nueva.",
        },
        {
            "name": "Tímido",
            "desc": "Es reservado con los extraños y necesita tiempo para adaptarse.",
        },
        {
            "name": "Energético",
            "desc": "Tiene mucha energía y es ideal para personas activas.",
        },
        {
            "name": "Leal",
            "desc": "Forma un fuerte vínculo con su familia y es muy protector.",
        },
    ]
...
def seed_dogs(behaviors, num_dogs=20):
    ...
    for i in range(num_dogs):
        ...
        DogImage.objects.create(dog=dog, image=placeholder_image_url, is_primary=True)
...
if __name__ == "__main__":
    ...
    if Dog.objects.exists():
        ...
        print(
            "Si quieres empezar de cero, descomenta la línea 'clear_data()' en el script."
        )
        print(
            "¿Quieres intentar ejecutar la seed con la data existente, con riesgo de error de duplicidad?"
        )
        response = input("y/n\n")
        if response == "y":
